...
/*
* 指定要捕获的异常(Shiro没有权限异常)
*
* */
    @ExceptionHandler(UnauthorizedException.class)
    public Object handleshiroException(Exception ex, HandlerMethod handlerMethod, HttpServletResponse response) throws IOException {//进入该方法,就代表捕获到异常
        //把异常信息打印出,方便我们开发的时候找bug
        ex.printStackTrace();
        //handlerMethod就是出现异常的处理方法
        //判断当前执行的方法是否是ajax的方法(@ResponseBody)
        if (handlerMethod.hasMethodAnnotation(ResponseBody.class)) {//如果是,则返回jsonresult
            JsonResult json = new JsonResult(false, "你没有权限进行操作");
            //使用response对象输出数据(json字符串)
            response.setContentType("application/json;charset=utf-8");
            response.getWriter().print(JSON.toJSONString(json));
            return null;
